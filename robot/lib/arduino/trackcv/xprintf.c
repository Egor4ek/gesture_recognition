...
static char *outptr;
int32_t (*xfunc_out)(unsigned char);
...
void xfputc (int32_t(*func)(unsigned char), char c)
{
	//if (_CR_CRLF && c == '\n') xputc('\r');		/* CR -> CRLF */

	if (func) func((unsigned char)c);
}
...
inline void xfputs (					/* Put a string to the specified device */
	int32_t (*func)(unsigned char),	/* Pointer to the output function */
	const char*	str				/* Pointer to the string */
)
{
...
void xfputsn (					/* Put a string to the specified device */
	int32_t(*func)(unsigned char),	/* Pointer to the output function */
	const char*	str,				/* Size of string */
	int32_t size
)
{
	int32_t i;
	int32_t (*pf)(unsigned char);
	
	pf = xfunc_out;		/* Save current output device */
	xfunc_out = func;	/* Switch output to specified device */
	for(i=0;i<size;i++)		/* Put the string */
		xputc(*str++);
...
)
{
	...
	for (;;) {
		...
		for (w = 0; c >= '0' && c <= '9'; c = *fmt++)	/* Minimum width */
			w = w * 10 + c - '0';
		if (c == 'l' || c == 'L') {	/* Prefix: Size is long int32_t */
			f |= 4; c = *fmt++;
		}
		if (!c) break;				/* End of format? */
		d = c;
		...
		case 'S' :					/* String */
			...
			continue;
		case 'C' :					/* Character */
			xputc((char)va_arg(arp, int32_t)); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
		...
		v = (f & 4) ? va_arg(arp, long) : ((d == 'D') ? (long)va_arg(arp, int32_t) : (long)va_arg(arp, uint32_t));
		if (d == 'D' && (v & 0x80000000)) {
			v = 0 - v;
			f |= 8;
		}
...
void xfvprintf (			/* Put a formatted string with va_arp to the specified device */
	int32_t(*func)(unsigned char),	/* Pointer to the output function */
	const char*	fmt,		/* Pointer to the format string */
	...
	for (;;) {
		...
		case 'S' :					/* String */
			...
			continue;
		case 'C' :					/* Character */
			xfputc(func,(char)va_arg(arp, int32_t)); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
